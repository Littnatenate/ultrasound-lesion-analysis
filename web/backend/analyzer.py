"""
FastAPI-adapted analyzer for ultrasound lesion detection.
Uses Detectron2 + PointRend with env-var configured paths.
"""
import os
import math
import cv2
...
# Add the project root to path so detectron2 and PointRend can be found
...
class Analyzer:
    def __init__(self, use_gpu=False):
        """Initializes the Detectron2 Predictor using env-var paths."""
        self.cfg = get_cfg()
        ...
        pointrend_config = os.environ.get(
            "POINTREND_CONFIG",
            os.path.join(PROJECT_ROOT, "projects", "PointRend", "configs",
                         "InstanceSegmentation", "pointrend_rcnn_R_50_FPN_3x_coco.yaml")
        )
        self.cfg.merge_from_file(pointrend_config)
    ...
    def _setup_datasets(self):
        ...
        train_dir = os.environ.get(
            "DATASET_DIR",
            r"C:\Breast cancer project (new)\Dataset\train"
        )
    ...
    def analyze_image(self, image_path, age):
        """
        Runs model inference and returns annotated image + results.
        """
