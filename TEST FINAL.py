...
def run_gui():
    ...
    intake_tab = tk.Frame(notebook, bg="#f0f2f5")
    result_tab = tk.Frame(notebook, bg="#f0f2f5")
    ...
    # ---------------- Patient Intake Tab ----------------
    intake_container = tk.Frame(
        intake_tab,
        bg="white",
        bd=0,
        highlightthickness=1,
        highlightbackground="#d1d9e0",
    )
    intake_container.pack(fill="both", expand=True, padx=20, pady=20)

    # Accent strip on intake card
    tk.Frame(intake_container, bg=ACCENT_COLOR, height=3).pack(fill="x", side="top")

    # Left panel
    left_panel = tk.Frame(intake_container, bg="white", padx=40, pady=30)
    ...
    file_row = tk.Frame(left_panel, bg="white")
    ...
    def on_run_analysis():
        ...
        if success:
            ...
            run_button.config(text="✓ Analysis complete", bg=OK_COLOR)
            status_label.config(text="Analysis complete.", fg=OK_COLOR)
            root.after(
                1500,
                lambda: (
    ...
    btns = tk.Frame(left_panel, bg="white")
    ...
    def clear_form():
        ...
        age_help_label.config(text="Enter a number (e.g., 45).", fg="#90949c")
        scan_help_label.config(text="Supported formats: PNG, JPG, BMP, TIF.", fg="#90949c")
        age_entry.config(highlightbackground="#d1d9e0", highlightcolor=ACCENT_COLOR)
        status_label.config(text="", fg="#90949c")
        update_run_button_state()
    ...
    def update_run_button_state(*args):
        ...
        if not age_str:
            valid = False
            age_help_label.config(text="Age is required.", fg=ERROR_COLOR)
            age_entry.config(highlightbackground=ERROR_COLOR, highlightcolor=ERROR_COLOR)
        else:
            try:
                float(age_str)
            except ValueError:
                valid = False
                age_help_label.config(text="Age must be numeric (e.g., 45).", fg=ERROR_COLOR)
                age_entry.config(highlightbackground=ERROR_COLOR, highlightcolor=ERROR_COLOR)
            else:
                age_help_label.config(text="Enter a number (e.g., 45).", fg="#90949c")
                age_entry.config(highlightbackground="#d1d9e0", highlightcolor=ACCENT_COLOR)
        ...
        if not img_path:
            valid = False
            scan_help_label.config(text="Ultrasound scan is required.", fg=ERROR_COLOR)
        else:
            scan_help_label.config(text="Supported formats: PNG, JPG, BMP, TIF.", fg="#90949c")

        if valid:
            run_button.config(state="normal", bg=ACCENT_COLOR)
            if status_label.cget("text") == "":
                status_label.config(text="Ready to analyze.", fg=OK_COLOR)
        else:
            run_button.config(state="disabled", bg="#bcc0c4")
    ...
    right_panel = tk.Frame(intake_container, bg="#f5f6f7", padx=24, pady=24)
    ...
    preview_card = tk.Frame(
        ...
        highlightthickness=1,
        highlightbackground="#d1d9e0",
        highlightcolor="#d1d9e0"
    )

    preview_card.pack(fill="both", expand=True)

    # accent strip on preview card
    tk.Frame(preview_card, bg=ACCENT_COLOR, height=3).pack(fill="x", side="top")

    image_frame = tk.Frame(preview_card, bg="white", padx=10, pady=10)
    ...
    def resize_preview_image(event=None):
        ...
        preview_label.config(image=preview_original_img["tk"], text="")
        preview_label.image = preview_original_img["tk"]

    # ---------------- Analysis Result Tab ----------------
    # ---------------- Analysis Result Tab ----------------
    result_header = tk.Frame(result_tab, bg="#f0f2f5")
    ...
    btn_row = tk.Frame(result_tab, bg="#f0f2f5")
    ...
    # Key stats row
    stats_row = tk.Frame(result_tab, bg="#f0f2f5")
    stats_row.pack(fill="x", padx=20, pady=(8, 0))

    stats_main_label = tk.Label(
        stats_row,
    ...
    tk.Frame(result_card, bg=ACCENT_COLOR, height=3).pack(fill="x", side="top")

    result_image_label = tk.Label(result_card, bg="white")
    result_image_label.pack(expand=True)

    # --- result image scaling ---
    def update_result_tab_image():
        if current_canvas["img"] is None:
            result_image_label.config(text="No analysis result yet.", image="")
        ...
        result_image_label.config(image=img_tk, text="")
